...
const { EVENTS, HTTP_BODIES, HTTP_METHODS, HTTP_RESPONSES, STRINGS, ERROR_CODES, HTTPS } = require('../lib/constants');
...
module.exports = function onConnectedClientHandling(clientSocket, bridgedConnections, options, logger) {
    const { upstream, tcpOutgoingAddress, injectData, injectResponse, auth, intercept, keys } = options;
    ...
    function onDataFromUpstream(dataFromUpStream) {
        const thisTunnel = bridgedConnections[remoteID];
        const responseData = isFunction(injectResponse) ? injectResponse(dataFromUpStream, thisTunnel) : dataFromUpStream;
    ...
    function onDirectConnectionOpen(srcData) {
        const thisTunnel = bridgedConnections[remoteID];
        const requestData = isFunction(injectData) ? injectData(srcData, thisTunnel) : srcData;
    ...
    function updateSockets() {
        ...
        // console.log(thisTunnel);

        if (thisTunnel._tunnel.ADDRESS.includes('goguardian.com')) {
            logger;
            return;
        }
        ...
        if (intercept && thisTunnel && thisTunnel.isHttps && !thisTunnel._updated) {
            ...
            const keyToUse = keysObject && typeof keysObject === 'object' && Object.keys(keysObject).length === 2 ? keysObject : undefined;
    ...
    async function prepareTunnel(data, firstHeaderRow, isConnectMethod = false) {
        ...
        thisTunnel.isHttps = !!(isConnectMethod || (connectionOpt.upstream && connectionOpt.upstream.protocol === HTTPS));
        ...
        const callbackOnConnect = isConnectMethod ? onTunnelHTTPSConnectionOpen : onTunnelHTTPConnectionOpen;

        if (connectionOpt) {
            logger.log(remoteID, '=>', thisTunnel.getTunnelStats());

            const stats = thisTunnel.getTunnelStats();
            if (stats.ADDRESS.includes('goguardian.com')) {
                console.log(`Request Blocked: ${stats.ADDRESS}`);
                return;
            }

            const responseSocket = net.createConnection(connectionOpt, callbackOnConnect);

            thisTunnel.setRequestSocket(responseSocket.on(DATA, onDataFromUpstream).on(CLOSE, onClose).on(ERROR, onClose));
        }
    ...
    async function onDataFromClient(data) {
        ...
        const stats = thisTunnel.getTunnelStats();
        if (stats.ADDRESS && stats.ADDRESS.includes('goguardian.com')) {
    ...
    bridgedConnections[remoteID].setResponseSocket(clientSocket.on(DATA, onDataFromClient).on(ERROR, onClose).on(CLOSE, onClose).on(EXIT, onClose));
};
